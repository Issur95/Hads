...
def resultsSinLogCorrect(request, question_id):
    question = get_object_or_404(Question, pk=question_id)
    return render(request, 'polls/correct.html', {'question': question})

def resultsSinLogIncorret(request, question_id):
    question = get_object_or_404(Question, pk=question_id)
    return render(request, 'polls/incorrect.html', {'question': question})
...
def vote(request, question_id):
    ...
    try:
        ...
        if request.user.is_authenticated():
            try:
                previous_correct_choice = p.choice_set.get(correctAnswer=True)
            except (Choice.DoesNotExist):
                previous_correct_choice = None

            if previous_correct_choice:
                previous_correct_choice.correctChoice = False
                previous_correct_choice.save()
            selected_choice.correctChoice = True
            selected_choice.save()
        else: 
            selected_choice.votes += 1
            selected_choice.save()
            if selected_choice.correctChoice==False:
        ...
        if request.method =='POST':
            ...
            if form.is_valid():
                ...
                if choice.correctChoice:
                    ...
                    for p in Choice.objects.filter(question=question_id):
                        p.correctChoice = False
                        p.save()
                choice.save()         
                #form.save()
        else: 
...
def index2(request):
    latest_question_list = Question.objects.filter(topic=request.POST['topoc'])
    template = loader.get_template('polls/index.html')
    context = {
                'title':'Lista de preguntas de la encuesta',
                'latest_question_list': latest_question_list,
